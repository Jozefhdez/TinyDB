...
## Features
- **SQL-like Commands**: `insert`, `select`, `select where`, `select where range`, `update`, and `delete` statements
...
- `delete <id>` - Delete a row by ID, shifting remaining cells left
- `select where id >= <id> and id <= <id>` - Fetch all rows within an ID range using B-tree traversal
...
- **Statement Parser** (`statement.c/h`): SQL command parsing and execution
  - INSERT, SELECT, SELECT WHERE, SELECT RANGE, UPDATE, and DELETE statement preparation
