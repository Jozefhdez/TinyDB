...
def test_insert_fails_when_table_full():
    # After implementing internal node splitting, the B-tree can handle many more inserts
    # This test now verifies that we can successfully insert 100+ rows
    commands = [f"insert {i} user{i} person{i}@example.com" for i in range(1, 101)]
    commands.append(".exit")
    result = run_script(commands)

    # Verify that all inserts were successful
    executed_count = sum(1 for line in result if line == "TinyDB > Executed.")
